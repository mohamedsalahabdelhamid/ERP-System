...
export default function Purchases() {
  ...
  const handleDelete = async (id, number) => {
    if (!window.confirm(`Delete draft purchase ${number}?`)) return;
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '1rem' }}>
        <button className="btn btn-primary" onClick={() => setShowForm(!showForm)}>+ New Purchase Invoice</button>
      </div>

      {showForm && (
        <div className="glass-card" style={{ marginBottom: '1.5rem' }}>
          <h3 style={{ marginBottom: '1.5rem' }}>New Purchase Invoice</h3>
          <form onSubmit={handleCreate}>
            ...
            <h4 style={{ margin: '1.5rem 0 1rem' }}>Purchase Lines</h4>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem' }}>
              <button type="submit" className="btn btn-primary" disabled={loading}>{loading ? 'Saving...' : 'Create Purchase Invoice'}</button>
      ...
      <div className="table-container">
        <table>
          <thead><tr><th>Invoice No</th><th>Date</th><th>Supplier</th><th>Currency</th><th>Total</th><th>Status</th><th>Action</th></tr></thead>
          <tbody>
            {invoices.length === 0
              ? <tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: '2rem' }}>No purchases found</td></tr>
              : invoices.map(inv => (
                <tr key={inv.id}>
                  <td>{inv.number}</td>
                  <td>{inv.date}</td>
                  <td>{partners.find(p => p.id === inv.partner_id)?.name || `Supplier #${inv.partner_id}`}</td>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      {!inv.is_confirmed && (
                        <>
                          <button className="btn btn-primary" style={{ padding: '0.25rem 0.75rem', fontSize: '0.8rem' }} onClick={() => handleConfirm(inv.id)}>Confirm & Stock In</button>
